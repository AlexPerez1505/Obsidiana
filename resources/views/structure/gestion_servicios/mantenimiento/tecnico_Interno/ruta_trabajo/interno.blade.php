...
@section('title', 'Ruta de Trabajo Interno - ' . ($service->service_number ?? $service->id))
...
@section('service_content')
    <style>
        ...
        .route-page-header .title-group h2 { margin: 0; font-size: 28px; color: #fff; }
        :root[data-theme="light"] .route-page-header .title-group h2 { color: var(--text); }
        .route-page-header .title-group p { margin: 4px 0 0; color: rgba(255,255,255,0.55); font-size: 14px; }
        ...
        .route-info-card {
            ...
            box-shadow: 0 8px 28px rgba(0,0,0,0.35), 0 0 14px rgba(168,85,247,0.2);
        }
        .route-info-card label { display: block; font-size: 12px; color: rgba(255,255,255,0.55); text-transform: uppercase; letter-spacing: .04em; margin-bottom: 6px; }
        .route-info-card span { font-size: 15px; font-weight: 700; color: #fff; }
        ...
        .timeline::before {
            ...
            background: rgba(168,85,247,0.35); border-radius: 2px;
        }
        .timeline-step {
            position: relative; margin-bottom: 22px; padding: 16px 18px;
            background: rgba(8,18,40,0.82); border: 1px solid rgba(168,85,247,0.35);
            border-radius: 14px; box-shadow: 0 8px 28px rgba(0,0,0,0.35);
        }
        .timeline-step::before {
            content: ''; position: absolute; left: -25px; top: 20px; width: 14px; height: 14px;
            border-radius: 50%; background: rgba(255,255,255,0.2); border: 2px solid rgba(168,85,247,0.55); z-index: 2;
        }
        .timeline-step.completed {
            border-color: rgba(168,85,247,0.75);
            background: rgba(168,85,247,0.08);
        }
        .timeline-step.completed::before { background: #a855f7; border-color: #a855f7; box-shadow: 0 0 10px rgba(168,85,247,0.65); }
        .timeline-step.current {
            border-color: #a855f7;
            background: rgba(168,85,247,0.12);
            box-shadow: 0 0 18px rgba(168,85,247,0.35);
        }
        .timeline-step.current::before { background: #a855f7; border-color: #fff; box-shadow: 0 0 16px rgba(168,85,247,0.85); animation: route-pulse 1.6s infinite; }
        .timeline-step.rejected {
            border-color: rgba(239,68,68,0.75);
            background: rgba(239,68,68,0.08);
        }
        .timeline-step.rejected::before { background: #ef4444; border-color: #ef4444; box-shadow: 0 0 10px rgba(239,68,68,0.65); }
        ...
        :root[data-theme="light"] .timeline-step.current { background: rgba(168,85,247,0.08); }
        @keyframes route-pulse {
            0%, 100% { transform: scale(1); }
            50% { transform: scale(1.15); }
        }
        .step-header { display: flex; align-items: center; justify-content: space-between; gap: 12px; flex-wrap: wrap; margin-bottom: 8px; }
        .step-order { font-size: 13px; font-weight: 800; color: #a855f7; }
        .step-title { margin: 0; font-size: 16px; font-weight: 700; color: #fff; flex: 1; }
        :root[data-theme="light"] .step-title { color: var(--text); }
        .step-meta { font-size: 13px; color: rgba(255,255,255,0.55); line-height: 1.5; }
    ...
    <div class="catalog-card service-section">
        <div class="route-page-header">
            <div class="title-group">
                <h2>Ruta de Trabajo Interno</h2>
                <p>{{ $service->service_number ?? 'NS-' . $service->id }} &mdash; Servicio interno</p>
            </div>
        ...
        <div class="route-info">
            ...
            <div class="route-info-card">
                <label>Técnico</label>
                <span>{{ trim(($service->internalTechnician->name ?? '') . ' ' . ($service->internalTechnician->last_name ?? '')) ?: 'N/A' }}</span>
            </div>
            <div class="route-info-card">
                <label>Equipo</label>
        ...
        <div class="timeline">
            @foreach ($steps as $step)
                ...
                <div class="timeline-step {{ $cssClass }}">
                    ...
                    <p class="step-meta">{{ $step->description ?? 'Sin descripción' }}</p>
                    @if ($tracking)
                        <p class="step-meta">
