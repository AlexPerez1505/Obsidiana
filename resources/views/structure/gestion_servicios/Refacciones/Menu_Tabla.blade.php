...
<div class="catalog-card service-section" style="padding:22px;">
    ...
    @if (session('success'))
        <div style="margin-bottom:16px; padding:12px 14px; border-radius:10px; background:rgba(34,197,94,0.14); border:1px solid rgba(34,197,94,0.4); color:#4ade80; font-size:14px;">
            {{ session('success') }}
        </div>
    @endif
        ...
        <table class="service-table" style="width:100%; border-collapse:collapse; font-size:14px;">
            <thead>
                <tr style="text-align:left; border-bottom:1px solid var(--border, rgba(255,255,255,0.1));">
                    ...
                    <th style="padding:12px 10px;">Compatible con</th>
            ...
            <tbody>
                @forelse($refacciones ?? [] as $refaccion)
                    <tr style="border-bottom:1px solid var(--border, rgba(255,255,255,0.06));">
                        ...
                        <td style="padding:12px 10px;">{{ $refaccion->name }}</td>
                        <td style="padding:12px 10px;">{{ $refaccion->subtype }}</td>
                        <td style="padding:12px 10px;">{{ $refaccion->description ?? '—' }}</td>
                        <td style="padding:12px 10px;">{{ number_format($refaccion->stock) }}</td>
                        <td style="padding:12px 10px;">${{ number_format($refaccion->price ?? 0, 2) }}</td>
                        <td style="padding:12px 10px;">{{ $refaccion->compatible_with ?? '—' }}</td>
                        <td style="padding:12px 10px;" class="muted">{{ $refaccion->created_at ? \Carbon\Carbon::parse($refaccion->created_at)->format('d/m/Y H:i') : '—' }}</td>
                        <td style="padding:12px 10px; text-align:center;">
                            <div class="action-menu" style="position:relative; display:inline-block;">
                                ...
                                <div class="action-dropdown" style="display:none; position:absolute; right:0; top:100%; margin-top:6px; background:var(--surface-2, #1f2937); border:1px solid var(--border, rgba(255,255,255,0.1)); border-radius:10px; box-shadow:0 10px 25px rgba(0,0,0,0.3); min-width:150px; z-index:50; overflow:hidden; text-align:left;">
                                    ...
                                    <form method="POST" action="{{ route('refacciones.destroy', $refaccion) }}" style="margin:0;" onsubmit="return confirm('¿Eliminar esta refacción?');">
                                        @csrf
                                        @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="10" style="padding:24px 10px; text-align:center; color:rgba(255,255,255,.55);">No hay refacciones registradas.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    @if (isset($refacciones) && $refacciones->hasPages())
