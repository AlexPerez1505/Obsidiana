            ...
            <tbody>
                @forelse ($cotizaciones as $cotizacion)
                    <tr style="border-bottom:1px solid var(--border, rgba(255,255,255,0.06));">
                        <td style="padding:12px 10px; font-weight:600; color:#00A8FF;">OS-{{ $cotizacion->id_servicio }}</td>
                        <td style="padding:12px 10px;">{{ trim(($cotizacion->cliente_nombre ?? '') . ' ' . ($cotizacion->cliente_apellido ?? '')) ?: '—' }}</td>
                        <td style="padding:12px 10px;">
                            @if ($cotizacion->tipo_cotizacion === 'externo')
                        ...
                        <td style="padding:12px 10px; text-transform:capitalize;">{{ str_replace('_', ' ', $cotizacion->estado) }}</td>
                        <td style="padding:12px 10px;">${{ number_format($cotizacion->total, 2) }}</td>
                        <td style="padding:12px 10px;" class="muted">{{ \Carbon\Carbon::parse($cotizacion->created_at)->format('d/m/Y H:i') }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="6" style="padding:24px 10px; text-align:center;" class="muted">No hay cotizaciones registradas.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
