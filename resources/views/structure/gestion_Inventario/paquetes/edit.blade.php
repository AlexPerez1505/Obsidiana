...
@section('content')
    <form method="POST" action="{{ route('inventory.paquetes.update', $paquete) }}">
        ...
        <x-ui.card style="margin-bottom:18px;">
            @php
                $idsSeleccionados = $paquete->productos->pluck('id')->toArray();
                $cantidadesPorProducto = $paquete->productos->pluck('pivot.cantidad', 'id');
            @endphp
            ...
            <div id="pchk-lista" class="pchk-grid">
                @foreach($productos as $producto)
                    @php
                        ...
                        $estaSeleccionado = in_array($producto->id, $idsSeleccionados);
                        $cantidadActual = $cantidadesPorProducto[$producto->id] ?? 1;
                    @endphp
                    <div class="pchk-item {{ $estaSeleccionado ? 'pchk-item--activo' : '' }}" data-search="{{ \Illuminate\Support\Str::lower($nombreProducto) }}">
                        <div class="pchk-item-row">
                            <input type="checkbox" id="pchk-{{ $producto->id }}" class="pchk-checkbox" data-id="{{ $producto->id }}" data-nombre="{{ $nombreProducto }}" @checked($estaSeleccionado)>
                        ...
                        <div class="pchk-qty" data-qty-for="{{ $producto->id }}" @if(!$estaSeleccionado) hidden @endif>
                            <span class="pchk-qty-label">Cantidad</span>
                            <div class="pchk-stepper">
                                <button type="button" class="pchk-qty-dec">−</button>
                                <input type="number" class="pchk-qty-input" name="productos[{{ $producto->id }}][cantidad]" value="{{ $cantidadActual }}" min="1" @disabled(!$estaSeleccionado)>
                                <button type="button" class="pchk-qty-inc">+</button>
                            </div>
                            <input type="hidden" class="pchk-id-input" name="productos[{{ $producto->id }}][id]" value="{{ $producto->id }}" @disabled(!$estaSeleccionado)>
                        </div>
                    </div>
                @endforeach
            </div>
