...
@section('content')
    ...
    <x-ui.page-header title="Nuevo Viático" :back="route('admin.viatics.index')" />
    ...
    <form method="POST" action="{{ route('admin.viatics.store') }}" enctype="multipart/form-data" id="vtForm">
        ...
        <x-ui.card style="margin-bottom:18px;">
            ...
            <div class="rgrid-2">
                <x-ui.form-group label="Lugar" name="place" placeholder="Ej. Guadalajara, Jalisco" />
                <x-ui.form-group for="vehicle_id" label="Vehículo">
                    <select id="vehicle_id" name="vehicle_id">
                        <option value="">Sin vehículo</option>
                        @foreach ($vehicles as $v)
                            <option value="{{ $v->id }}" @selected((string) old('vehicle_id') === (string) $v->id)>
                                {{ $v->model ?: $v->brand ?: 'Vehículo' }}
                            </option>
                        @endforeach
                    </select>
                </x-ui.form-group>
                <x-ui.form-group label="Fecha" name="expense_date" type="date" :value="old('expense_date', now()->format('Y-m-d'))" />
            </div>
        </x-ui.card>
        ...
        <x-ui.card style="margin-bottom:18px;">
            ...
            <div class="rgrid-2">
                <x-ui.form-group label="Casetas" name="tolls" type="number" step="0.01" min="0" placeholder="0.00" inputmode="decimal" />
                <x-ui.form-group label="Gasolina" name="fuel" type="number" step="0.01" min="0" placeholder="0.00" inputmode="decimal" />
                <x-ui.form-group label="Viáticos" name="meals" type="number" step="0.01" min="0" placeholder="0.00" inputmode="decimal" />
                <x-ui.form-group label="Hospedaje" name="lodging" type="number" step="0.01" min="0" placeholder="0.00" inputmode="decimal" />
                <x-ui.form-group label="Adicional" name="additional" type="number" step="0.01" min="0" placeholder="0.00" inputmode="decimal" />
            </div>
            <x-ui.form-group label="Descripción" for="description">
                <textarea id="description" name="description" rows="3" placeholder="Describe el motivo del viaje o gastos adicionales...">{{ old('description') }}</textarea>
            </x-ui.form-group>
        </x-ui.card>
